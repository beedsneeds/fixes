    ...
    while iteration < args.train_iters:
        if args.profile and torch.distributed.get_rank() in args.profile_ranks:
            if args.use_pytorch_profiler:
                ...
                torch.cuda.nvtx.range_push("custom_profile_region")
                torch.cuda.cudart().cudaProfilerStart()
                # Leave autograd on if you want fine grained data. Incldue a .__exit__()
                # added the custom push above instead
                # torch.autograd.profiler.emit_nvtx(record_shapes=True).__enter__()
                profile_open = True
        if profile_open: 
            print_rank_0(f'Collecting traces on iteration {iteration}')
        ...
        try: 
            ...
            if args.profile and profile_open and \
            (iteration - 1) == args.profile_step_end and \
            torch.distributed.get_rank() in args.profile_ranks:
                if args.use_pytorch_profiler:
                    assert prof is not None
                    prof.stop()
